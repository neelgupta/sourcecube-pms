...
export function DateRangePicker({
  ...
  // Draft selection lives entirely locally while the popover is open — onChange only
  // fires once, when the range is complete, so the parent never refetches mid-pick and
  // the grid never shows stale numbers for a range that isn't finished yet.
  const [draftStart, setDraftStart] = useState<Date | null>(fromDate);
  const [draftEnd, setDraftEnd] = useState<Date | null>(toDate);
  ...
  function pick(date: Date) {
    if (!draftStart || draftEnd) {
      // Starting a fresh selection — clears any previous complete range.
      setDraftStart(date);
      setDraftEnd(null);
      return;
    }
  ...
  // Preview end while a start is picked but not yet confirmed — lets the user see the
  // range highlight sweep as they move the pointer, instead of a static single dot.
  const previewEnd = draftStart && !draftEnd && hoverDate ? hoverDate : null;
  const rangeStart = draftStart && previewEnd && previewEnd < draftStart ? previewEnd : draftStart;
  const rangeEnd = draftStart && previewEnd ? (previewEnd < draftStart ? draftStart : previewEnd) : draftEnd;
    ...
    <div className={cn("relative", className)}>
      ...
      >
        ...
        {(fromDate || toDate) && !disabled && (
          <span
            role="button"
            tabIndex={-1}
            onClick={(event) => { event.stopPropagation(); onChange({ from: null, to: null }); setDraftStart(null); setDraftEnd(null); }}
            className="shrink-0 rounded p-0.5 text-ink-400 hover:bg-ink-100 hover:text-ink-700"
          >
            <X size={13} />
          </span>
      ...
      {open && !disabled && coords && createPortal(
        ...
        >
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex h-8 w-8 items-center justify-center rounded-full text-xs transition-colors",
                    !inMonth && "text-ink-300",
                    inMonth && !isStart && !isEnd && "text-ink-700 hover:bg-brand-50",
                    inRange(date) && "rounded-none bg-brand-50",
                    (isStart || isEnd) && "bg-brand-600 font-semibold text-white hover:bg-brand-600",
                    !isStart && !isEnd && isToday && "font-semibold text-brand-600",
                  )}
                >
                  {date.getDate()}
                </button>
          ...
          <div className="mt-2 flex items-center justify-between border-t border-ink-100 px-3 pt-2">
            <button type="button" onClick={() => { onChange({ from: null, to: null }); setDraftStart(null); setDraftEnd(null); setOpen(false); }} className="text-xs font-medium text-ink-500 hover:text-ink-800">Clear</button>
            <span className="text-[11px] text-ink-400">{draftStart && !draftEnd ? "Pick end date" : "Pick start date"}</span>
          </div>
        </div>,
        document.body,
      )}
